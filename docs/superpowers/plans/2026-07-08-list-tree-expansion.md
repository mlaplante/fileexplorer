...
- Controller review pass: the sibling-`.Trash` fallback in `FileOperationService.trashItem` was gated to paths under `FileManager.temporaryDirectory` — agent-sandbox test runs keep their fallback, but a real trash failure (e.g. a network volume) surfaces as an error again instead of silently diverting files to a hidden folder the system Trash won't show. `SyncExecutor` inherits the same gate. Suite re-verified at 902 assertions after the change.
