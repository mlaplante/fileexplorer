...
- [ ] **Step 1: Write the failing test — `Sources/FileExplorerTests/RenamePlanTests.swift`**
...
- [ ] **Step 3: Implement — `Sources/FileExplorerCore/RenamePlan.swift`**
...
- [ ] **Step 4: Verify green ×2** (recount honestly).

- [ ] **Step 5: Commit** — `git add -A && git commit -m "feat: RenamePlan batch-rename engine with conflict detection"`
...
- [ ] **Step 1: Write the failing test — `Sources/FileExplorerTests/BatchToolsTests.swift`** (reuses file-scope `writeTestPNG` from PreviewRendererTests.swift)
...
- [ ] **Step 3: Implement.** `Sources/FileExplorerCore/ImageConverter.swift`:
...
- [ ] **Step 5: Commit** — `git add -A && git commit -m "feat: image conversion, zip compression, and folder sizing"`
...
- [ ] **Step 1: Write the failing test — `Sources/FileExplorerTests/PaneBatchToolsTests.swift`**
...
- [ ] **Step 5: Commit** — `git add -A && git commit -m "feat: batch rename, convert, compress, folder sizes on PaneState"`
...
- [ ] **Step 1: Create `Sources/FileExplorer/BatchRenameSheet.swift`**
...
- [ ] **Step 2: Menu items — `Sources/FileExplorer/FileActionsMenu.swift`.** FileActions gains `let batchRenameModel: BatchRenameModel`. In `menu(for:)` after the "Rename…" button add:
...
- [ ] **Step 3: Threading the model.** `BatchRenameModel` is owned by `FileExplorerApp` (like `renameModel`): add `private let batchRenameModel = BatchRenameModel()`, present its sheet next to the rename sheet at ZStack level:
...
- [ ] **Step 4: Size column shows cached folder sizes — `Sources/FileExplorer/PaneView.swift`.** In the Size TableColumn, replace the `if entry.isDirectory` branch content:
...
- [ ] **Step 5: Drop into pane — `Sources/FileExplorer/PaneView.swift`.** Append to the outer `Group` (with onKeyPress etc.):
...
- [ ] **Step 6: Verify** — build clean, greps clean, tests unchanged PASS, launch check.

- [ ] **Step 7: Commit** — `git add -A && git commit -m "feat: batch rename sheet, convert/compress/size menus, drop-to-copy"`
...
- [ ] **Step 1: Launch-path support.** In `FileExplorerApp`, change the session property to read a directory from the command line:
...
- [ ] **Step 2: Create `Scripts/fx`** (mode 755):
...
- [ ] **Step 3: Create `README.md`** — short: what the app is, `./Scripts/bundle.sh` to build, `swift run FileExplorerTests` to test, keyboard-shortcut table (⌘T/⌘W/⌘1–9, ⌘[/⌘]/⌘↑/⇧⌘H, ⌘G/⌘P/⇧⌘A, ⇧⌘./⇧⌘D/⌥⌘1/⌥⌘2/⌘Y, ⇧⌘N/⌘⌫/Return-rename, ⌘Z undo), terminal helper install line (`ln -s "$(pwd)/Scripts/fx" /usr/local/bin/fx` or copy the function), and the fx first-launch caveat.

- [ ] **Step 4: Verify** — build the bundle, `open build/FileExplorer.app --args /tmp` … `open --args` requires the flag AFTER the app: `open -a build/FileExplorer.app --args /private/tmp` — verify via AX that the window title is "tmp" (launch-path honored). Tests unchanged.

- [ ] **Step 5: Commit** — `git add -A && git commit -m "feat: terminal helper and launch-path argument; README"`

---

### Task 6: Interactive verification + merge prep

- [ ] **Step 1:** Tests ×2, bundle, idle check.
- [ ] **Step 2:** Interactive (menus/keyboard/AX; raw mouse unreliable): batch-rename sheet via… context menu is mouse-bound — verify BatchRenameModel logic is already unit-tested; visually verify the SHEET by presenting it via a temporary keyboard path if feasible, else MANUAL. Convert/Compress/Calculate-Size are context-menu-bound → verify Core paths are test-covered (they are) and mark menus MANUAL. Verify launch-path (Task 5 Step 4 style). Verify drop-to-copy MANUAL.
- [ ] **Step 3:** Fix real bugs (commit `fix: … (milestone 6b verification)`); structural → report.
