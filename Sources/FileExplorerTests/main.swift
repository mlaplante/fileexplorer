import Foundation

await directoryLoaderTests()
await fileSorterTests()
await treeFlattenerTests()
await treeExpansionTests()
await navigationHistoryTests()
await directoryWatcherTests()
await paneStateTests()
await ancestorChainTests()
await filterPresetsTests()
await filterEngineTests()
await paneFilterTests()
await tabStateTests()
await sessionStateTests()
await recentsTests()
await scannerTests()
await paletteModelTests()
await previewRendererTests()
await hoverPreviewModelTests()
await fileOperationTests()
await operationConflictPlannerTests()
await operationQueueStateTests()
await aliasTests()
await macPlatformToolsTests()
await advancedSystemTests()
await undoTests()
await renamePlanTests()
await batchToolsTests()
await paneBatchToolsTests()
await newFolderWithSelectionTests()
await grouperTests()
await sessionSnapshotTests()
await sortMenuTests()
await trashRegistryTests()
await volumeSpaceTests()
await locationsBuilderTests()
await sessionPersisterTests()
await sessionAutosaverTests()
await selectionResolverTests()
await dropDecisionTests()
await settingsModelTests()
await scriptInvocationPlannerTests()
await scriptListerTests()
await scriptResultFormatterTests()
await scriptRunnerTests()
await renameExecutorTests()
await fuzzyMatcherTests()
await collisionNamerTests()
await dailyOpsTests()
await archiveTests()
await archiveCatalogParserTests()
await archiveCatalogTests()
await archiveExtractorTests()
await archiveBrowserModelTests()
await commentTests()
await infoGathererTests()
await tagFilterTests()
await filterPresetTests()
await contentScannerTests()
await folderComparatorTests()
await syncExecutorTests()
await renameTokensTests()
await imageResizerTests()
await fileHasherTests()
await usageRankingTests()
await usageScannerTests()
await duplicateKeepPlannerTests()
await duplicateFinderTests()
await updateCheckerTests()
await shortcutTests()
await columnsModelTests()
await rubberBandTests()
await gridNavigatorTests()
await springLoadTests()
await gitStatusParserTests()
await gitStatusIndexTests()
await gitRepoLocatorTests()
await gitStatusModelTests()
await benchStatsTests()

finish()
